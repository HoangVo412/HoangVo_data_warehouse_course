WITH fact_stock_item_transaction__source AS(
  SELECT *
  FROM `duckdata-320210.wide_world_importers.warehouse__stock_item_transactions`
)

, fact_stock_item_transaction__rename_column AS (
  SELECT
    ...
    , quantity
    , last_edited_by 
  FROM fact_stock_item_transaction__source
)

, fact_stock_item_transaction__cast_type AS (
  SELECT
    CAST(stock_item_transaction_id AS INTEGER) AS stock_item_transaction_id
  ...
  FROM fact_stock_item_transaction__rename_column 
)

SELECT
*
FROM fact_stock_item_transaction__cast_type
